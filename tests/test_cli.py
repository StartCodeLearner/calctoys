...
def test_list_command_json_output(capsys):
    ...
    assert "cylinder div2-ext-pressure" in commands
...
def test_cylinder_div2_ext_pressure_json(capsys):
    rc, out, _ = _run_cli(
        [
            "cylinder", "div2-ext-pressure",
            "--E-y", "29000000", "--D-o", "24", "--t", "0.5",
            "--L", "120",
            "--sigma-ys", "30000", "--sigma-uts", "60000",
            "--material-type", "ferritic_steel", "--json",
        ],
...
def test_cylinder_div2_ext_pressure_longer_shell_lower_allowable(capsys):
    # Longer unsupported length -> lower allowable external pressure
    args_base = [
        "cylinder", "div2-ext-pressure",
        "--E-y", "29000000", "--D-o", "24", "--t", "0.5",
        "--sigma-ys", "30000", "--sigma-uts", "60000",
    ...
    _, out_short, _ = _run_cli(args_base + ["--L", "60"], capsys)
    _, out_long, _ = _run_cli(args_base + ["--L", "240"], capsys)
    Pa_short = json.loads(out_short)["P_a"]
    Pa_long = json.loads(out_long)["P_a"]
    assert Pa_short > Pa_long > 0
